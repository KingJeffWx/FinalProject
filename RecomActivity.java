...
public class RecomActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        responseView = (TextView) findViewById(R.id.recomResponse);
        movie = (EditText) findViewById(R.id.editText5);
        submit = (Button) findViewById(R.id.button5);
        submit.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                new RecomActivity.SearchJSON().execute();
                new RecomActivity.ParseJSON().execute();
            }
        });
    }
    ...
    class ParseJSON extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... urls) {
            try {
                URL url = new URL("https://api.themoviedb.org/3/movie/" + movie_id + "/recommendations?api_key=a62a54c8895971fd4fbb1c92d2445164&language=en-US&page=1");
                HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
                try {
                    BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(urlConnection.getInputStream()));
                    StringBuilder stringBuilder = new StringBuilder();
